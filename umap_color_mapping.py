...
# 重新標準化資料，包含新的分類列
logging.info('重新對資料進行標準化')
data_scaled = scaler.fit_transform(data_numeric)
data_scaled_df = pd.DataFrame(data_scaled, columns=numeric_columns)
logging.info('資料重新標準化完成')

# 5. 準備資料進行 UMAP 降維
logging.info('準備資料進行 UMAP 降維')
# 選擇新的分類列進行分析
category_columns = list(set(category_mapping.values()))
data_for_umap = data_scaled_df[category_columns]

# 為每個參與者找到攝入量最高的分類
data['主要分類'] = data[category_columns].idxmax(axis=1)
...
# 7. 創建存放圖片的資料夾
output_dir = 'umap_visualizations_with_categories'
...
for n_neighbors, min_dist in product(n_neighbors_list, min_dist_list):
    logging.info(f'Processing parameter combination: n_neighbors={n_neighbors}, min_dist={min_dist}')
    # 建立 UMAP 模型
    reducer = umap.UMAP(
